...
event_no_fun = function(tpr_flag, display_flag, flyer_flag){
  
  n <- length(tpr_flag)
  Event_Num <- numeric(n)
  k <- 1
  
  # Pad with 0 at the beginning for easier comparison
  tpr_padded <- c(0, tpr_flag)
  display_padded <- c(0, display_flag)
  flyer_padded <- c(0, flyer_flag)
  
  for (j in 2:length(tpr_padded)) {
    
    # Create a "state" identifier for the combination of all three flags
    current_state <- paste0(tpr_padded[j], display_padded[j], flyer_padded[j])
    prev_state <- paste0(tpr_padded[j-1], display_padded[j-1], flyer_padded[j-1])
    
    # Only create events when TPR=1
    if (tpr_padded[j] == 1) {
      
      # Start a new event if:
      # 1. TPR turns on (0->1), OR
      # 2. Display turns on (0->1) - Display ALWAYS creates a new event, OR
      # 3. Flyer turns on (0->1), OR
      # 4. State changes (combination changes)
      if (tpr_padded[j-1] == 0 ||  # TPR turns on
          (display_padded[j] == 1 && display_padded[j-1] == 0) ||  # Display turns on (NEW EVENT)
          (flyer_padded[j] == 1 && flyer_padded[j-1] == 0) ||  # Flyer turns on
          (tpr_padded[j-1] == 1 && current_state != prev_state)) {  # State changes
        Event_Num[j-1] <- k
        k <- k + 1
      } 
      # Continue existing event if TPR stays on and state stays the same
      else if (tpr_padded[j-1] == 1 && current_state == prev_state) {
        Event_Num[j-1] <- k - 1
      }
    } 
    # No event if TPR is 0
    else {
      Event_Num[j-1] <- 0
    }
  }
  
  # Remove events that appear only once (single-day events)
  # Count occurrences of each event number
  event_counts <- table(Event_Num[Event_Num > 0])
  
  # Find events that appear only once
  single_occurrence_events <- as.numeric(names(event_counts[event_counts == 1]))
  
  # Remove single-occurrence events
  if (length(single_occurrence_events) > 0) {
    Event_Num[Event_Num %in% single_occurrence_events] <- 0
  }
  
  # Renumber remaining events to be consecutive (removes gaps)
  unique_events <- unique(Event_Num[Event_Num > 0])
  if (length(unique_events) > 0) {
    event_map <- setNames(seq_along(unique_events), unique_events)
    Event_Num[Event_Num > 0] <- event_map[as.character(Event_Num[Event_Num > 0])]
  }
...
event_multiplier_fun = function(ppg_events){
  ...
  ppg_df[,Inc_Display :=Inc_TPR_Display]
  ...
  ppg_df[,Total_Units_Sales := Base_Units + Inc_TPR_Only + Inc_Display+Inc_TPR_Feature+Inc_TPR_Feature_Display+Inc_TPR_Display]
  ppg_df[,Total_Inc_Units :=  Inc_TPR_Only + Inc_Display+Inc_TPR_Feature+Inc_TPR_Feature_Display+Inc_TPR_Display]
  ...
  ppg_df[, Event_Multiplier_Flyer:=Inc_TPR_Feature/Base_Units]
  ppg_df[, Event_Multiplier_Flyer_Display:=Inc_TPR_Feature_Display/Base_Units]
  ...
  return(list(ppg_df$Event_Multiplier, ppg_df$Event_Multiplier_Display,ppg_df$Event_Multiplier_Flyer,ppg_df$Event_Multiplier_Flyer_Display, ppg_df$Event_Multiplier_Discount,
...
data_prep <- function(nielsen,model_results,event_r,maping,cost_bible,tesco_slots,ean_to_ldesc_map,dl_nl,lsm_new,promo_seq,retailer_end_day,retailer,manuf,brand,date_maping){
  ...
  nielsen1 = subset(nielsen1, nielsen1$`Week End Date` %in% latest_104_weeks)
  rm(latest_104_weeks)
  ...
  ean_to_ldesc = ean_to_ldesc_map[,.(ItemID...1,ITEM_NAME)]
  ...
  nielsen_including_model_betas = merge(nielsen1,model_results,by.x = "ITEM", by.y = "UPC_EAN_Betas",
                                        all.x = T)
  
  
  nielsen_including_model_betas
  rm(nielsen1)
  ...
  #master_data$`Feature Only_Betas`=1.5
  #master_data$`Feature and Display_Betas`=1.5
  ...
  master_data[,"Inc TPR only"] = (master_data$`Price Change Index`^master_data$`Promoted Price Elasticity_Betas` - 1)*
    master_data$`ACV Distribution (C) (TPR)`*master_data$`Base Units`/master_data$`ACV Distribution`
  ...
  master_data[,"Inc TPR + Display Only"] = (((master_data$`Price Change Index`^master_data$`Promoted Price Elasticity_Betas`)*
                                               master_data$`Display Only_Betas`) - 1)*master_data$`ACV Distribution (C) (TPR and Display)`*master_data$`Base Units`/master_data$`ACV Distribution`
  ...
  master_data[,"Inc Total Feature Only"] = (master_data$`Feature Only_Betas` - 1)*master_data$`ACV Distribution (C) (feature only)`*
    master_data$`Base Units`/master_data$`ACV Distribution`
  
  master_data[,"Inc Feature + Display Only"] = (master_data$`Feature and Display_Betas`- 1)*master_data$`ACV Distribution (C) (feature and displa`*
    master_data$`Base Units`/master_data$`ACV Distribution`
  
  master_data[,"Inc Total Display Only"] = (master_data$`Display Only_Betas` - 1)*master_data$`ACV Distribution (C) (display only)`*
    master_data$`Base Units`/master_data$`ACV Distribution`
  ...
  cols = c("Inc TPR only","Inc TPR + Feature Only","Inc TPR + Display Only","TPR+Feature+Dsiplay","Inc Multibuy + Feature Only","Inc Multibuy + Display Only",
           "Inc Multibuy + Feature + Display","Inc Total Multibuy Only","Inc Total Feature Only","Inc Feature + Display Only")
  ...
  master_data[,"Total Incremental Unit Sales"] =  master_data[,"Inc TPR only"]+master_data[,"Inc TPR + Feature Only"]+
    ...
    master_data[,"Inc Total Feature Only"]+master_data[,"Inc Feature + Display Only"] +master_data[,"Inc Total Display Only"]
  
  #master_data[,"Inc Display Only"] = master_data$`Total Incremental Unit Sales` - master_data$`Inc TPR only`         #addon
  
  #V.IMP
  Inc_cols = c("Inc TPR only","Inc Total Display Only","Total Incremental Unit Sales","TPR+Feature+Dsiplay","Inc Feature + Display Only","Inc Total Feature Only" )
  ...
  master_ppg = merge(master_data,maping, by.x = "ITEM", by.y = "Item_Code", all.x = T)
  
  rm(master_data)
  master_ppg$PPG_Parent=master_ppg$PPG
  ...
  #master_ppg <- master_ppg[master_ppg$PPG == "D77", ]
  #4------------------------------------------------RB FINANCIAL-----------------------------------------------
  ...
  cost_bible$MRRP_Unit=as.numeric(cost_bible$MRRP_Unit)
  #4.3) Join master_ppg with cost_bible
  master_cost = merge(master_ppg, cost_bible, by.x = "PPG", by.y = "PPG", all.x = T)         #change
  ...
  event$`Week End Date`= dmy(event$`Week End Date`)
  #event$"Week End Date"=as.Date(event$`Week End Date`)
  event$`Net invetsments_HEA`=as.numeric(event$`Net invetsments_HEA`)
  master_fund$ITEM=as.numeric(master_fund$ITEM)
  maping$Item_Code=as.numeric(maping$Item_Code)
  event$ITEM=as.numeric(event$ITEM)
  #  maping$ITEM=as.numeric(maping$EAN)
  
  master_fund$`Week End Date` <- as.Date(master_fund$`Week End Date`)
  # Doing it for 1 ppg
  #master_fund$`Week End Date`=as.Date(master_fund$`Week End Date`)
  
  master_with_flag = merge(master_fund, event, all.x = T,by = c("ITEM","Week End Date"))
  master_with_flag[is.na(master_with_flag)]=0
  #write.csv(master_with_flag,"master_with.csv")
  master_with_flag <- master_with_flag[complete.cases(master_with_flag)]
  master_with_flag$Flag_HEA=as.numeric(master_with_flag$Flag_HEA)
  ...
  master_with_flag$Flag_Display_HEA=as.numeric(master_with_flag$Flag_Display_HEA)
  master_with_flag$Flag_Flyer_HEA=as.numeric(master_with_flag$Flag_Flyer_HEA)
  
  #6.11) Making sure retro funding is 0 where promotion was not there
  master_with_flag[,Retro_Fund_Total := Retro_Fund_Total*Flag_HEA]
  master_with_flag[,Retro_Fund_Unit := Retro_Fund_Unit*Flag_HEA]
  master_with_flag[,`Total Incremental Unit Sales` := `Total Incremental Unit Sales` * Flag_HEA]
  master_with_flag[,`Inc TPR only` := `Inc TPR only`*Flag_TPR_HEA]                                                     #addon
  master_with_flag[,`Inc Total Display Only` := `Inc Total Display Only`*Flag_Display_HEA]                                         #addon
  master_with_flag[,`Inc Total Feature Only` := `Inc Total Feature Only`*Flag_Flyer_HEA]                                         #addon
  master_with_flag[, `Inc Feature + Display Only` :=`Total Incremental Unit Sales`*Flag_Display_HEA *Flag_Flyer_HEA]
  #additional calculations for roi
  #additional columns for roi
  ...
  master_with_flag_2=master_with_flag
  ...
  #write.csv(shiny_ip_nielsen,"nil.csv")
  
   
  #write.csv(shiny_ip_nielsen,"shiny.csv")
  #Calculate Elasticities for PPG, to be used to create Event List
  ppg_elasticity = master_with_flag_2[(Flag_RB_Financial == 1 & Flag_Modeled_Betas==1 & Flag_HEA ==1 ),  
  ...
  ppg_elasticity[,weight := Units/sum(Units), by = .(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT)]
  
  ppg_elasticity = ppg_elasticity[,.(`Total Regular Price Elasticiy_Betas` = sum(`Total Regular Price Elasticiy_Betas`*weight),
  ...
  write.csv(ppg_elasticity,"ppg.csv")
  ppg_elasticity
  ...
  master_with_flag[,weight := Units/sum(Units), by = .(PPG,`Week End Date`)] 
  ...
  event_list = master_with_flag[,.(RSP_Unit = sum(RSP_Unit*weight),Flag_Display_HEA = max(Flag_Display_HEA),Flag_TPR_HEA = max(Flag_TPR_HEA),Flag_Flyer_HEA = max(Flag_Flyer_HEA),Flag_HEA=max(Flag_HEA),
                                   Base_Units = sum(`Base Units`) ,Retro_Fund_Total =  sum(Retro_Fund_Total),
                                   Promo_Price = sum(Promo_Price*weight), Flag_Modeled_Betas = max(Flag_Modeled_Betas), Flag_PPG = max(Flag_PPG),
                                   VAT = max(VAT), Net_Cost_Unit = max(Net_Cost_Unit), FM_Abs_Unit_2 = sum(FM_Abs_Unit_2),
                                   Net_Cost_Total = sum(Net_Cost_Total), COGS_Total = sum(COGS_Total), Units = sum(Units),FM_Abs_Unit_1 = sum(FM_Abs_Unit_1),
                                   Flag_RB_Financial = max(Flag_RB_Financial), `Base Units` = sum(`Base Units`),COGS_Unit = max(COGS_Unit),
                                   Gross_Sales = sum(Gross_Sales), Retailer_Revenue = sum(Retailer_Revenue),
                                   FM_Total = sum(FM_Total), No_Of_Units = mean(No_Of_Units), Display_Cost =max(`Net invetsments_HEA`), 
                                   Display_Num = first(Display_Num), OID_Total = sum(OID_Total), UNCR_Total = sum(UNCR_Total),
                                   Total_Inc_Unit_Sales = sum(`Total Incremental Unit Sales`),
                                   Inc_Unit_Discount = sum(`Inc TPR only`), Inc_Unit_Display = sum(`Inc Total Display Only`),Inc_Unit_Feature= sum(`Inc Total Feature Only`),Inc_Unit_Feature_Display=sum(`Inc Feature + Display Only`),
                                   ACV_D = sum(`ACV Distribution`*weight), NIS = sum(NIS),
                                   ...
                                   `Multi Buy_Betas` = sum(`Multi Buy_Betas`*weight),
                                   `Total Multibuy Effect_Betas` = sum(`Total Multibuy Effect_Betas`*weight),
                                   R_GM_Inc = sum(R_GM_Inc), R_Net_Revenue_Inc = sum(R_Net_Revenue_Inc), R_NIS_Inc = sum(R_NIS_Inc),
                                   R_Total_Trade_Inv_Inc = sum(R_Total_Trade_Inv_Inc), Value = sum(Value)
  ), 
  by = .(PPG,`Week End Date`,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description, `DISPLAY TYPE_HEA`)]
  
  event_list[is.na(event_list)] = 0 
  event_list$Flag_Display_HEA
  #---------------------------Calculations-------------------------------------
  ...
  df_list <- lapply(split(event_list, event_list$Key), function(key_data) {
    # Sort by date to ensure chronological order
    key_data <- key_data[order(key_data$`Week End Date`), ]
    
    # Call event_no_fun with all three flags
    key_data$Event_Number <- event_no_fun(
      tpr_flag = key_data$Flag_TPR_HEA,
      display_flag = key_data$Flag_Display_HEA,
      flyer_flag = key_data$Flag_Flyer_HEA
    )
  ...
  ly_kpi = ly_kpi[,.(PPG,`Week End Date`,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description,Inc_GM_Abs,Trade_Investment,Units,
                     Gross_Sales,Net_Cost_Total,GM_Abs, FM_Total, Retailer_Revenue, Net_Revenue, Base_Units, Total_Inc_Unit_Sales, 
                     No_Of_Units, ACV_D,ACV_D_Any_Promo, ACV_D_Multibuy_Display, ACV_D_Multibuy_Only, ACV_D_Feature_Display,
                     ACV_D_Display_Only, ACV_D_Feature_Only, ACV_D_Unsupported, ACV_D_Total_Multibuy, ACV_D_Multibuy_Feature_Display,
                     ACV_D_Multibuy_Feature, `Total Regular Price Elasticiy_Betas`,`Regular Price Elasticity_Betas`,
                     `Reg Price Ratio Rest of Brand_Betas` , `Reg Price Ratio Rest of Manufacturer_Betas`,
                     `Reg Price Ratio Rest of Category_Betas` , `Promoted Price Elasticity_Betas` ,`Display Only_Betas`,
                     `Feature Only_Betas`,`Feature and Display_Betas`, `Multi Buy_Betas`, `Total Multibuy Effect_Betas`,
                     Flag_Display_HEA, Flag_Flyer_HEA, Flag_TPR_HEA, Display_Num, Inc_Revenue, NIS, Inc_NIS, Display_Cost, Retro_Fund_Total,
                     VAT,R_GM_Inc,R_Net_Revenue_Inc,R_NIS_Inc,R_Total_Trade_Inv_Inc, Value
  )]
  ...
  exclude_ppg = df[,.(PPG,`Week End Date`,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description,Inc_GM_Abs,Trade_Investment,Units,
                      Gross_Sales,Net_Revenue,GM_Abs,Promo_Price,RSP_Unit,VAT, Net_Cost_Unit, FM_Total, Retailer_Revenue, 
                      Base_Units, No_Of_Units,NIS, Inc_NIS, Inc_Revenue, R_GM_Inc,R_Net_Revenue_Inc,R_NIS_Inc,
  ...
  event_list[,weight := (Units)/sum(Units), by = .(PPG, Event_Number)]
  ...
  event_list$Flag_Flyer_HEA
  Events = event_list[,.(RSP_Unit = sum(RSP_Unit*weight), Promo_Price = sum(Promo_Price*weight),Flag_Display_HEA = max(Flag_Display_HEA),
                         Flag_TPR_HEA = max(Flag_TPR_HEA),Flag_Flyer_HEA=max(Flag_Flyer_HEA),Total_Inc_Unit_Sales = sum(Total_Inc_Unit_Sales),Flag_RB_Financial = 
                           max(Flag_RB_Financial),Base_Units = sum(Base_Units), Inc_GM_Abs = sum(Inc_GM_Abs), 
                         Retro_Fund_Total = sum(Retro_Fund_Total), Display_Cost = max(Display_Cost), Display_Type, first(`DISPLAY TYPE_HEA`) ,Trade_Investment = 
                           sum(Trade_Investment), Flag_Modeled_Betas = max(Flag_Modeled_Betas), Flag_PPG = max(Flag_PPG),
                         Start_Date = min(`Week End Date`), End_Date = max(`Week End Date`), FM_Total = sum(FM_Total),
                         Retailer_Revenue = sum(Retailer_Revenue), No_Of_Units = mean(No_Of_Units),
                         Display_Num = first(Display_Num),Display_Type=max(`DISPLAY TYPE_HEA`), Inc_Revenue = sum(Inc_Revenue), NIS = sum(NIS), Inc_NIS = sum(Inc_NIS),
                         Inc_Unit_Discount = sum(Inc_Unit_Discount), Inc_Unit_Display = sum(Inc_Unit_Display),Inc_Unit_Feature=sum(Inc_Unit_Feature),Inc_Unit_Feature_Display =sum(Inc_Unit_Feature_Display),
                         R_GM_Inc = sum(R_GM_Inc), R_Net_Revenue_Inc = sum(R_Net_Revenue_Inc), R_NIS_Inc = sum(R_NIS_Inc),
  ...
  by = .(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT, Event_Number, PPG_Description, Display_Num, Display_Type)]
  ...
  Events[,Event_Multiplier_Display := Inc_Unit_Display/Base_Units] 
  Events[,Event_Multiplier_Flyer:= Inc_Unit_Feature]
  Events[,Event_Multiplier_Flyer_Display:= Inc_Unit_Feature_Display]
  ...
  #                 .(LY_Investment = sum(Trade_Investment), LY_Any_Promo_Weeks = .N, LY_Display_Weeks = sum(unique(Flag_Display_HEA)),LY_Flyer_Weeks = sum(unique(Flag_Flyer_HEA))),
  
  #                by = .(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT, PPG_Description)]
  
   
  ly_kpi_2 = e_list[Flag_Modeled_Betas == 1 & Flag_PPG == 1 & Flag_RB_Financial == 1 & Flag_TPR_HEA == 1,
                    .(LY_Investment = sum(Trade_Investment), 
                      LY_Any_Promo_Slots = uniqueN(Event_Number),  # Count distinct promo events
                      LY_Display_Slots = uniqueN(Event_Number[ Flag_Display_HEA == 1]),  # Count distinct events with display
                      LY_Flyer_Slots = uniqueN(Event_Number[Flag_Flyer_HEA == 1])
                      ),  # Count distinct events with flyer
  ...
  latest_rsp = latest_rsp[order(`Week End Date`, decreasing = TRUE)]
  latest_rsp = latest_rsp[, .SD[1], by = .(PPG, `SECTOR 2`, `TRADING COMPANY`, `PRODUCT RANGE`, FORMAT, PPG_Description)]
  
  lsm_subset = lsm_new[,.(PPG, LSM_Promo_Price_Min, Min_Total_Weeks, Max_Total_Weeks, Non_LSM_Min_Total_Weeks,
  ...
  lsm_subset = merge(lsm_subset,map, by.x = "PPG", by.y = "PPG")
  
  #setnames(lsm_subset,"PPG","PPG_Parent")
  #setnames(lsm_subset,"PPG.y","PPG")
  
  lsm_subset = merge(lsm_subset, latest_rsp[,.(PPG, RSP_Unit)], by = "PPG")
  ...
  if (toupper(retailer) == "TESCO") {
    ...
    tesco_events = event_list[Flag_Modeled_Betas == 1 & Flag_PPG == 1 & Flag_RB_Financial == 1 ,
                              .(PPG,`Week End Date`,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description,
                                Flag_TPR_HEA,Flag_Display_HEA,Flag_Flyer_HEA,Event_Number, Display_Type,
    ...
    events_to_map = events_shiny[,.(PPG, `SECTOR 2`, `TRADING COMPANY`,`PRODUCT RANGE`,
                                    FORMAT,PPG_Description, Event_Number, Display_Cost)]
    ...
    final_tesco_calendar =
      tesco_week_cal[,.(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description,Tesco_Week_No, Tesco_Week_Ending,Base_Units_Tesco,
    ...
    tesco_event_base_units = tesco_week_cal[, .(Base_Units_Tesco = sum(Base_Units_Tesco)),
                                            by = .(PPG, `SECTOR 2`, `TRADING COMPANY`,
                                                   `PRODUCT RANGE`, FORMAT,PPG_Description,
                                                   Event_Number_Tesco)]
    ...
    tesco_events = event_list[Flag_Modeled_Betas == 1 & Flag_PPG == 1 & Flag_RB_Financial == 1 ,
                              .(PPG,`Week End Date`,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description,Flag_TPR_HEA,Flag_Flyer_HEA,Flag_Display_HEA,Event_Number, Display_Type,
    ...
    tesco_week_cal = merge(tesco_week_cal, events_to_map, by = "Key", all.x = T,allow.cartesian = T)
    
    #Make NA as 0
    tesco_week_cal[is.na(tesco_week_cal)] = 0
    
    #8.1.3 Divide Retro Funding, Display Cost, Display Type, Inc Sales equally
    tesco_week_cal[,  Display_Cost := Display_Cost , by = .(Key)]
    
    
    #Get cost bible kpi's
    tesco_week_cal = merge(tesco_week_cal, cost_bible, by = "PPG", all.x = T,allow.cartesian = T)
    ...
    final_tesco_calendar = 
      tesco_week_cal[,.(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description,Tesco_Week_No,`Week End Date`,Base_Units,
                        RSP_Unit,Net_Cost_Unit,COGS_Unit,VAT,BIP_Case,OID,No_Of_Units,STP_Unit,UNCR_Unit,OID_Unit,FM_Abs_Unit_1,
                        `FM%`,Discount,Display_Type, Display_Cost,Event_Multiplier, Flag_Display_HEA,Flag_TPR_HEA,Flag_Flyer_HEA,
    ...
    names(final_tesco_calendar) = c("PPG","Category","Manufacturer","Brand","Format","PPG_Description","Tesco_Week_No","Week_Ending","Base_Units","RSP_Unit",
                                    "Net_Cost_Unit","COGS_Unit","VAT","BIP_Case","OID","No_Of_Units","STP_Unit","UNCR_Unit","OID_Unit",
                                    "FM_Abs_Unit_1","FM%","Discount","Display","Display_Cost","Event_Multiplier_Tesco","Display_Flag",
                                    "TPR_Flag","Flyer_Flag","Event_Lift","Total_Sales","Promo_Price","Retro_Funding_Unit","Retro_Funding_Total",
                                    "UNCR_Total","OID_Total","Total_Trade_Investment","Gross_Sales","Net_Revenue","NIS","BIP","COGS_Total",
                                    "GM_Abs","Inc_GM_Abs","FM_Abs_Unit_2","FM%_2","FM_Total","Retailer_Revenue","Inc_NIS","Inc_Revenue",
                                    "Units_Sale_in_Case","R_NIS_Inc", "R_Net_Rev_Inc", "R_GM_Inc", "R_Trade_Inv_Inc")
  ...
  #Create Daily Date
  base_sales = master_with_flag[Flag_PPG == 1 & Flag_RB_Financial == 1,.(ITEM,`Base Units`,PPG,`Week End Date`,
                                                                         `SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description)]
  
   
  #dl_nl$EAN = as.character(dl_nl$EAN)
 
  dl_nl$EAN=as.numeric(dl_nl$EAN)
  dl_nl$ITEM= dl_nl$EAN
  base_sales = merge(base_sales,dl_nl, by="ITEM",all.x = T)
  ...
  base_sales = base_sales[,.(Base_Units = sum(`Base Units`)), by = .(PPG,`Week End Date`,`SECTOR 2`,
                                                                     `TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description)]
  ...
  #base_tmp
  base_tmp = unique(base_sales[,.(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description)])
  ...
  base_sales_tmp = base_sales[,.(Base_Units,Key)]
  ...
  # Get the most recent RSP per PPG (handle cases where PPG might not be in latest week)
  # Option 1: Use most recent non-zero RSP per PPG
  # RB_Fin = RB_Fin[RSP_Unit > 0]  # Remove zero RSPs first
  RB_Fin = RB_Fin[order(`Week End Date`, decreasing = TRUE)]
  RB_Fin = RB_Fin[, .SD[1], by = .(PPG, `SECTOR 2`, `TRADING COMPANY`, `PRODUCT RANGE`, FORMAT, PPG_Description)]
  RB_Fin = RB_Fin[,.(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description,RSP_Unit)]
  ...
  var = cost_bible[,.(PPG,Net_Cost_Unit,COGS_Unit,VAT, BIP_Case, OID, No_Of_Units, STP_Unit, UNCR_Unit, OID_Unit)]
  RB_Fin  = merge(RB_Fin, var, by.x = "PPG", by.y = "PPG", all.x = T)
  ...
  ppg_zero_rsp = RB_Fin[RSP_Unit == 0,.(Key)]
  ...
  master_zero_rsp_prom = master_zero_rsp_prom[,.(Value = sum(Value),Units = sum(Units),Price = sum(Value)/sum(Units)), 
                                              by = .(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description,ITEM)]
  
  #roll to PPG level
  master_zero_rsp_prom[,weight := Units/sum(Units), by = .(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description)]
  master_zero_rsp_prom = master_zero_rsp_prom[,.(Promo_Price = sum(Price*weight)), 
                                              by = .(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description) ]
  ...
  master_zero_rsp_not_prom = master_zero_rsp_not_prom[,.(Value = sum(Value),Units = sum(Units),Price = sum(Value)/sum(Units)), 
                                                      by = .(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description,ITEM)]
  
  #roll to PPG level
  master_zero_rsp_not_prom[,weight := Units/sum(Units), by = .(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description)]
  master_zero_rsp_not_prom = master_zero_rsp_not_prom[,.(Not_Promo_Price = sum(Price*weight)), 
                                                      by = .(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description) ]
  ...
  ppg_zero_rsp = merge(ppg_zero_rsp, master_zero_rsp_not_prom[,.(Key,Not_Promo_Price)], all.x = T, by = "Key" )
  ...
  #date_maping$current_year_date = mdy(date_maping$current_year_date)    #addon
  #date_maping$last_year_date = mdy(date_maping$last_year_date)          #addon
  ...
  unique(event_optim$PPG_Description)
  ...
  for (ppg in ppg_list) {
    ...
    one_ppg[, Event_Multiplier_Flyer := Inc_TPR_Feature/Base_Units]
    one_ppg[, Event_Multiplier_Flyer_Display := Inc_TPR_Feature_Display/Base_Units]
  ...
  additional_events = additional_events[,.(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description,RSP_Unit,Net_Cost_Unit ,
                                           COGS_Unit,Promo_Price,Event_Multiplier,Event_Multiplier_Display,Event_Multiplier_Flyer,Event_Multiplier_Flyer_Display,Event_Multiplier_Discount,
  ...
  additional_events[,Flyer_Flag := 0]
  additional_events[,Flyer := ""]
  additional_events[,Flyer_Cost := 0]
  ...
  e1 = additional_events[,.(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description,Discount,Display,Display_Cost,Display_Flag,Flyer_Flag,Flyer,Flyer_Cost,
                            Event_Multiplier,Event_Multiplier_Discount, Event_Multiplier_Display,Event_Multiplier_Flyer,Event_Multiplier_Flyer_Display,Event_Multiplier_Tesco, 
  ...
  #12.4.1 For Nielsen
  df = event_multiplier_fun(display_events)
   
  #12.4.2 Place in the data
  df
  display_events[,Event_Multiplier := df[[1]] ] ; display_events[,Event_Multiplier_Display := df[[2]] ] ;
  display_events[,Event_Multiplier_Discount := df[[5]] ]
  display_events[,Event_Multiplier_Flyer:=df[[3]]];display_events[,Event_Multiplier_Flyer_Display:=df[[4]]]; display_events[,ROI_GM := df[[6]] ];
  display_events[,ROI_Rev := df[[10]] ] ; display_events[,ROI_NIS := df[[11]] ]
  display_events[,R_ROI_GM := df[[12]] ] ; display_events[,R_ROI_Rev := df[[13]] ]
  display_events[,R_ROI_NIS := df[[14]] ]
  
  
  #12.4.3 Calculate event multiplier for tesco
  
  display_events[,Event_Multiplier_Tesco := df[[7]] ] ; display_events[,Event_Multiplier_Display_Tesco := df[[8]] ] ;
  display_events[,Event_Multiplier_Discount_Tesco := df[[9]] ] 
  #display_events$`DISPLAY TYPE_HEA`
  display_events[,Display_Flag := 1]
  display_events[,Flyer_Flag := 0]
  display_events[,Flyer:= ""]
  display_events[,Flyer_Cost:= 0]
  ...
  #till here bilal
  e2 = display_events[,.(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description,Discount,Display,Display_Cost,Display_Flag,Flyer_Flag,Flyer,Flyer_Cost,
                         Event_Multiplier,Event_Multiplier_Discount, Event_Multiplier_Display,Event_Multiplier_Flyer,Event_Multiplier_Flyer_Display,Event_Multiplier_Tesco, 
                         Event_Multiplier_Discount_Tesco, Event_Multiplier_Display_Tesco,
                         ROI_GM,ROI_Rev,ROI_NIS,Promo_Price,RSP_Unit,R_ROI_GM,R_ROI_Rev,R_ROI_NIS)]
  ...
  df = event_multiplier_fun(additional_d_events)
  additional_d_events[,Event_Multiplier := df[[1]] ] ; additional_d_events[,Event_Multiplier_Display := df[[2]] ] ;
  additional_d_events[,Event_Multiplier_Discount := df[[5]] ];
  
  additional_d_events[,Event_Multiplier_Flyer:=df[[3]]];additional_d_events[,Event_Multiplier_Flyer_Display:=df[[4]]]; 
  additional_d_events[,ROI_GM := df[[6]] ];
  additional_d_events[,ROI_Rev := df[[10]] ] ; additional_d_events[,ROI_NIS := df[[11]] ]
  additional_d_events[,R_ROI_GM := df[[12]] ] ; additional_d_events[,R_ROI_Rev := df[[13]] ]
  additional_d_events[,R_ROI_NIS := df[[14]] ]
  
  
  #12.4.3 Calculate event multiplier for tesco
  
  if (toupper(retailer) == "TESCO") {
    additional_d_events[,Event_Multiplier_Tesco := df[[7]] ] ; additional_d_events[,Event_Multiplier_Display_Tesco := df[[8]] ] ;
    additional_d_events[,Event_Multiplier_Discount_Tesco := df[[9]] ] }
  else {
  ...
  additional_d_events = additional_d_events[,Flyer_Flag := 0]
  additional_d_events = additional_d_events[,Flyer := ""]
  additional_d_events = additional_d_events[,Flyer_Cost := 0]
  #write.csv(additional_d_events,"3 Addtional Display Events_.csv", row.names = F)
  
  e3 = additional_d_events[,.(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description,Discount,Display,Display_Cost,Display_Flag,Flyer_Flag,Flyer,Flyer_Cost,
                              Event_Multiplier,Event_Multiplier_Discount, Event_Multiplier_Display,Event_Multiplier_Flyer,Event_Multiplier_Flyer_Display,Event_Multiplier_Tesco, 
                              Event_Multiplier_Discount_Tesco, Event_Multiplier_Display_Tesco,
                              ROI_GM,ROI_Rev,ROI_NIS,Promo_Price,RSP_Unit,R_ROI_GM,R_ROI_Rev,R_ROI_NIS)]
  ...
  # ========== Create e4: Flyer events (TPR + Flyer only, no display) ==========
  # Similar to e1 but with flyer flag - HISTORICAL FLYER EVENTS
  
  # Get flyer events from historical data (similar to how e2 gets display events)
  
  
   flyer_events_historical = master_with_flag[Flag_RB_Financial == 1 & Flag_Modeled_Betas==1 & Flag_HEA ==1 & 
                                               Flag_Flyer_HEA == 1 & Flag_Display_HEA == 0,
                                             .(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description,Units,
                                               `Base Units`,Base_Units_Tesco,Flag_Flyer_HEA,
                                               `ACV Distribution`,`ACV Distribution (C) (any promo)`,
                                               `ACV Distribution (C) (multibuy and displ`, `ACV Distribution (C) (multibuy only)`,
                                               `ACV Distribution (C) (feature and displa`, `ACV Distribution (C) (display only)`,
                                               `ACV Distribution (C) (feature only)`,`ACV Distribution (C) (Unsupported)`,`ACV Distribution (C) (total multibuy)`,
  ...
  flyer_events_historical = flyer_events_historical[,weight := Units/sum(Units), 
                                                     by = .(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description)]
   
  flyer_events_historical = flyer_events_historical[,.(Flag_Flyer_HEA = max(Flag_Flyer_HEA),
                                      Base_Units = sum(`Base Units`), Base_Units_Tesco = sum(display_events$Base_Units_Tesco),
                                      Units = sum(Units), 
                                      ACV_D = sum(`ACV Distribution`*weight), 
                                      ACV_D_Any_Promo = sum(`ACV Distribution (C) (any promo)`*weight),
   ...
   by = .(PPG, `SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,
          FORMAT,PPG_Description)]
  
  # Aggregate by PPG (similar to e2 aggregation)
  
  # Merge with elasticities
 
  flyer_events_historical = merge(flyer_events_historical, ppg_elasticity, 
                                  by = c("PPG", "SECTOR 2","TRADING COMPANY","PRODUCT RANGE","FORMAT","PPG_Description"), 
                                  all.x = T)
  
  # Get latest RSP
 flyer_events_historical <- flyer_events_historical[complete.cases(flyer_events_historical), ]
  flyer_events_historical[,Key := paste(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description)]
  flyer_events_historical = merge(flyer_events_historical,RB_Fin[,.(Key,RSP_Unit,Net_Cost_Unit,`FM%`,VAT,COGS_Unit, OID_Unit, UNCR_Unit,STP_Unit)], 
                                  by = "Key", all.x = T)
  
  
  # Generate promo prices for each PPG
  empty_df_flyer = data.table()
  for (i in 1:nrow(flyer_events_historical)){
    ppg_events = flyer_events_historical[i, ]
    min_promo = 0.5
    max_promo = floor(as.numeric(ppg_events$RSP_Unit)*2)/2
    max_promo = ifelse(max_promo == as.numeric(ppg_events$RSP_Unit),max_promo - 0.5,max_promo)
    Promo_Price = seq(from = min_promo, to = max(max_promo), by = 0.5)
    ppg_events = cbind(ppg_events, Promo_Price)
    empty_df_flyer = rbind(empty_df_flyer, ppg_events)
  }
  
  flyer_events_historical = empty_df_flyer

   rm(min_promo,max_promo,Promo_Price, ppg_events, empty_df_flyer)
   
  # Calculate event multipliers
  flyer_events_historical[,Display_Cost:=0]
  df = event_multiplier_fun(flyer_events_historical)
  flyer_events_historical[,Event_Multiplier := df[[1]] ] 
  flyer_events_historical[,Event_Multiplier_Display := df[[2]]]
  
  flyer_events_historical[,Event_Multiplier_Flyer := df[[3]]]
  flyer_events_historical[,Event_Multiplier_Flyer_Display := df[[4]]]
  flyer_events_historical[,Event_Multiplier_Discount := df[[5]] ]
  flyer_events_historical[,ROI_GM := df[[6]] ]
  flyer_events_historical[,ROI_Rev := df[[10]] ] 
  flyer_events_historical[,ROI_NIS := df[[11]] ]
  flyer_events_historical[,R_ROI_GM := df[[12]] ] 
  flyer_events_historical[,R_ROI_Rev := df[[13]] ]
  flyer_events_historical[,R_ROI_NIS := df[[14]] ]
  
  # For Tesco
  if (toupper(retailer) == "TESCO") {
    flyer_events_historical[,Event_Multiplier_Tesco := df[[7]] ] 
    flyer_events_historical[,Event_Multiplier_Discount_Tesco := df[[9]] ]
  } else {
    flyer_events_historical[,Event_Multiplier_Tesco := 0 ] 
    flyer_events_historical[,Event_Multiplier_Discount_Tesco := 0] 
    flyer_events_historical[,Event_Multiplier_Display_Tesco := 0] 
  }
  
  flyer_events_historical[,Display := " "]
  flyer_events_historical[,Display_Cost := 0]
  flyer_events_historical[,Display_Flag := 0]
  flyer_events_historical[,Flyer := "Flyer"]
  flyer_events_historical[,Flyer_Cost := 0]
  flyer_events_historical[,Flyer_Flag := 1]
  flyer_events_historical[,Discount := 1- (Promo_Price/RSP_Unit)]
  
  # Create e4
   
  e4 = flyer_events_historical[,.(PPG,`SECTOR 2`,`TRADING COMPANY`,`PRODUCT RANGE`,FORMAT,PPG_Description,Discount,Display,Display_Cost,Display_Flag,Flyer_Flag,Flyer,Flyer_Cost,
                                  Event_Multiplier,Event_Multiplier_Discount, Event_Multiplier_Display,Event_Multiplier_Flyer,Event_Multiplier_Flyer_Display,Event_Multiplier_Tesco, 
                                  Event_Multiplier_Discount_Tesco, Event_Multiplier_Display_Tesco, ROI_GM,ROI_Rev,ROI_NIS,Promo_Price,RSP_Unit,R_ROI_GM,R_ROI_Rev,R_ROI_NIS)]
  
  e4[,Flag_Event_Exist_in_History := 1]
  e4[,Flag_Historical_Display_Event := 0]
  
  # Historical flyer event
  
  # ========== Create e5: Flyer + Display events (TPR + Display + Flyer) ==========
  # Similar to e2 but with flyer added - HISTORICAL FLYER + DISPLAY EVENTS
  
  # Get flyer + display events from historical data
  
  final_events <- rbind(e1, e2, e3, e4)
  final_events[is.na(final_events)] <- 0
